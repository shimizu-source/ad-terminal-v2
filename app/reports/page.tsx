...
function getStatus(report: Report) {
  ...
  if (summary.includes("変更を検知")) {
    return "変更あり";
  }

  if (
    summary.includes("取得に失敗") ||
    summary.includes("エラーが発生")
  ) {
    return "取得失敗";
  }

  if (summary.includes("初回保存")) {
    return "初回保存";
  }
...
export default async function ReportsPage() {
  ...
  if (error) {
    console.error("レポート一覧取得失敗:", error);
  }

  const reports = (data || []) as Report[];

  const changedCount = reports.filter(
    (report) => getStatus(report) === "変更あり"
  ).length;

  const errorCount = reports.filter(
    (report) => getStatus(report) === "取得失敗"
  ).length;
